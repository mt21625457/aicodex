...
- `model-provider-routing`：provider 配置可再次选择 Chat Completions wire API；`wire_api = "chat"` 反序列化不再以硬错误拒绝；默认仍为 Responses。远程/托管 thread config（proto `enum WireApi`）同步扩展 `WIRE_API_CHAT`。
...
- 不受影响（本变更范围外）：
  ...
  - 远程/托管 thread config proto（`codex.thread_config.v1.proto` 的 `enum WireApi`）：新增 `WIRE_API_CHAT`；未知 wire_api 仍报错
- 主要风险：
