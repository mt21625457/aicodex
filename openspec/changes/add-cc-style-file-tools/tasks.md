...
- [x] 0.1 Human review approves `proposal.md`, `design.md`, and all three specs
      before any Rust implementation begins.
- [x] 0.2 Confirm the finite policy truth table:
...
- [x] 0.3 Confirm the locked schema/limits from design §5: `path`, 1-based
      ...
      receipt-store caps.
- [x] 0.4 Confirm raw-byte SHA-256 receipt identity and the shared round-trip
...
- [x] 1.1 Add a focused `file_tools/` module under
...
- [x] 1.2 Resolve model path strings against the selected environment's
...
- [x] 1.3 Implement a bounded, turn-scoped receipt store containing fixed-size
...
- [x] 1.4 Extend `codex-file-system` with `write_file_conditional` and typed writes:
...
- [x] 1.5 Add internal exec-server RPC `fs/conditionalWriteFile` and extend
...
- [x] 1.6 Implement `read_file` through `ExecutorFileSystem` metadata/stream APIs
      ...
      reached.
- [x] 1.7 Extract or reuse a shared round-trip text representation used by both
...
- [x] 1.8 Implement `edit_file` exact replacement + `replace_all`. Require every
...
- [x] 1.9 Implement `write_file` create and overwrite. Require full receipt
      coverage + `MatchSha256` for overwrite and atomic `MustNotExist` for create;
      refresh/invalidate the receipt after completion.
- [x] 1.10 Extract a shared reviewable file-mutation path used by ApplyPatch,
      ...
      precondition checks.
- [x] 1.11 Add the default-disabled typed feature config
...
- [x] 2.1 Unit-test exact schemas, fixed output header, every design §5 boundary,
...
- [x] 2.5 Test two overwrite mutations emitted in one batch; they MUST snapshot
      the prior-step receipt and MUST NOT let the first completion refresh state
      that silently authorizes the second.
- [x] 2.6 Test an external modification while approval is pending; executor-side
...
- [ ] 2.7 Test local, sandboxed, remote Linux, and Wine/Windows conditional writes.
      ...
      support.
- [x] 2.8 Test UTF-8 BOM/CRLF and a representative existing legacy encoding
      round trip, plus safe rejection of UTF-16/unsupported/binary inputs.
- [x] 2.9 Add core integration coverage with
...
- [x] 2.10 Assert gate-off excludes `read_file`/`edit_file`/`write_file` from both
...
- [x] 2.11 If new tool lifecycle text or names render in the TUI, add/update and
      review the required `insta` snapshots.

## 3. Phase B — Compatible and Kimi K3 Auto Rollout

- [x] 3.1 Complete the typed `[features.dedicated_file_tools]` config with
...
- [x] 3.2 In `auto`, advertise dedicated tools for Claude Compatible providers
...
- [x] 3.3 In Anthropic `auto`, advertise the supported native `text_editor` as
      the sole primary editor, keep ApplyPatch hidden, and decouple native editor
      capability from ApplyPatch model visibility.
- [x] 3.4 Add `dedicated` and `dedicated_with_apply_patch` Anthropic opt-in modes
...
- [x] 3.5 Keep OpenAI Responses and Chat wire behavior unchanged; this proposal
      MUST NOT advertise dedicated file tools there.
- [x] 3.6 Generate model-facing prefer-dedicated guidance only from the actual
...
- [x] 3.7 Do not add general product documentation under this repository's
      `docs/`; record the rollout gate, mode truth table, remote-path semantics,
      safety limits, and rollback behavior as an official documentation follow-up.
- [x] 3.8 Run `cd codex-rs && just write-config-schema` and include the updated
      config schema fixture.

## 4. Phase B Request and Tool-Loop Verification

- [x] 4.1 Assert the complete request/registry truth table for rollout-off,
...
- [x] 4.2 Claude mock integration: Compatible `read_file` then `edit_file`
      succeeds and the next request contains a non-error `tool_result`.
- [x] 4.3 Claude mock integration: missing/stale receipt fails without disk
      mutation and returns a model-correctable read-again result.
- [x] 4.4 Resume integration: existing hidden ApplyPatch/native handlers execute
...
- [x] 4.5 Assert dynamic guidance is present only when all dedicated tools are
...
- [x] 5.1 During implementation, run targeted tests for every changed crate:
      ...
      - `cd codex-rs && just test -p codex-apply-patch` only if that crate
        changed.
- [x] 5.2 Ask before running the complete `cd codex-rs && just test` suite if
      common/core/protocol changes require it.
- [x] 5.3 Before finalizing each large Rust phase, run the appropriately scoped
      `cd codex-rs && just fix -p <project>`, then `cd codex-rs && just fmt`.
      Do not rerun tests after the final fix/fmt pass.
- [x] 5.4 Run `openspec validate add-cc-style-file-tools --strict` and review the
      actual diff against the phase's model-visibility invariant.
- [x] 5.5 If any Rust dependency manifest changes, run repository-root
