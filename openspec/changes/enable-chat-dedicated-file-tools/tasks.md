...
- [x] 0.1 Human review approves this change's `proposal.md`, `design.md`, both
      specs, and `tasks.md` before implementation.
- [x] 0.2 Confirm `add-cc-style-file-tools` Phase A is approved, implemented,
...
- [x] 0.3 Lock `ChatFileToolMode` values and default:
      `legacy` (default), `dedicated`, `dedicated_with_apply_patch`.
- [x] 0.4 Confirm the locked review decisions: top-level config placement,
...
- [x] 1.1 Add typed `ChatFileToolMode`; reject unknown values and do not model
      the policy as multiple booleans. Store it as top-level
      `chat_file_tool_mode` in `ConfigToml`/resolved Config.
- [x] 1.2 Default to `legacy`; existing Chat request/tool exposure MUST remain
      deep-equal when the field is omitted.
- [x] 1.3 For a Chat non-legacy mode, reject resolved session config when the
...
- [x] 1.4 In `dedicated`, register `read_file` / `edit_file` / `write_file` as
      direct and ApplyPatch as hidden dispatch.
- [x] 1.5 In `dedicated_with_apply_patch`, advertise all three dedicated tools
      plus ApplyPatch. Keep this mode explicit and non-default.
- [x] 1.6 Apply this policy only for `WireApi::Chat`; add isolation assertions
      that Responses and Claude model-visible tools remain unchanged.
- [x] 1.7 Resolve and lock mode when a new session is created. Config changes do
...
- [x] 2.1 Reuse `create_tools_json_for_chat_completions`, `chat_tool_name`, and
      `ChatToolCallInfo`; do not special-case dedicated wire names or change the
      hash/sanitization algorithm.
- [x] 2.2 Assert each mapped dedicated declaration is a Chat function tool with
      the dependency proposal's object schema and stays inside per-tool/total
      Chat context budgets.
- [x] 2.3 Add `ChatFileToolGuidance` under `core/context` and implement
...
- [x] 3.2 Assert tool reordering and ApplyPatch visibility do not change the
      mapped names of dedicated tools.
- [x] 3.3 Assert mapped Chat names reverse to semantic
      `read_file` / `edit_file` / `write_file` with
      `ChatToolCallKind::Function`.
- [x] 3.4 Assert prior hidden ApplyPatch assistant calls serialize with their
...
- [x] 3.5 Assert a provider/schema/tool-budget construction error is actionable
      and does not trigger an automatic legacy request.
- [x] 3.6 Assert unknown mode fails config loading; Chat non-legacy with a disabled
...
- [x] 3.7 Assert guidance stays below its hard cap, remains deep-equal across
      retries/steps, and is absent in legacy even when same-named dynamic tools
      are present.

## 4. Mock End-to-End Chat Tool Loops

- [x] 4.1 Stream fragmented mapped `read_file` arguments, reconstruct the
...
- [x] 4.2 Execute `read_file -> edit_file -> edit_file` across Chat sampling
      steps; assert first and second edits succeed and the committed file matches.
- [x] 4.3 Execute `write_file(create) -> edit_file`; assert create establishes the
      dependency proposal's full receipt and immediate refinement succeeds.
- [x] 4.4 Execute missing/stale receipt cases; assert failure is returned as a
      model-correctable Chat tool result, the file is unchanged, and the turn can
      continue after a new read.
- [x] 4.5 Exercise multiple tool calls and assert dependent calls cannot use a
...
- [x] 4.6 Verify hidden dispatch can complete a valid resumed legacy call without
      advertising the hidden tool in the next request.

## 5. Remote, Docs, and Rollback

- [x] 5.1 Build core integrations with
...
- [x] 5.4 Do not add general product documentation under this repository's
      `docs/`; record the mode truth table, hard dependency, provider caveat,
      mapped-name/context-fragment behavior, session scope, staged errors, and
      rollback to `legacy` as an official documentation follow-up.
- [x] 5.5 Run `cd codex-rs && just write-config-schema` and include
      `codex-rs/core/config.schema.json` when config shape changes.
- [x] 5.6 Test that switching a new session back to `legacy` restores the prior
      Chat declarations without removing shared hidden handlers or rewriting old
      history.

## 6. Quality Gates

- [x] 6.1 During implementation run `cd codex-rs && just test -p codex-core`.
- [x] 6.2 Run `cd codex-rs && just test -p codex-tools` only if that crate changes.
- [x] 6.3 Ask before running the complete `cd codex-rs && just test` suite if
      core/common/protocol changes require it.
- [x] 6.4 Before finalizing, run scoped `cd codex-rs && just fix -p <project>`,
