...
> （Phase A）实现并验证前，**不得开始本变更的 Rust 实现或改变 Chat model-visible
> 工具面**。
...
- 为 Chat wire 增加独立 typed policy：
  ...
  - `dedicated_with_apply_patch`：同时广告 dedicated 工具与 `apply_patch`，用于显式
    fallback/诊断
- `chat_file_tool_mode != legacy` 必须依赖 `dedicated_file_tools` rollout gate 和三个
  handler 已注册；依赖不满足时配置/启动失败并给出可行动错误，不静默降级。
...
- 仅当三个 dedicated 工具实际出现在 Chat request 时，追加使用实际 mapped wire
  names 的 prefer-dedicated 指引；不得只告诉模型调用一个请求中不存在的未映射名称。
...
- **Hard dependency**：`add-cc-style-file-tools`
  - Phase A hidden handlers、bounded turn receipt、environment-aware PathUri、共享
    reviewable mutation 与 local/remote tests 已实现
...
- 预计受影响：
  - `codex-rs/core` — Chat tool visibility policy、mapped-name guidance、spec-plan
    truth table、mock tool loop tests
  - `codex-rs/tools` — 原则上只增加 dedicated Chat serialization assertions；仅当
    现有普通 function-tool serializer 暴露真实缺口时才改实现
  - `codex-rs/features` / `codex-rs/core/config.schema.json` — typed Chat mode 与 schema
...
- Mitigations：
  ...
  - 依赖调用必须跨 Chat completion 顺序执行；失败保持无写入并返回可纠正 tool result
