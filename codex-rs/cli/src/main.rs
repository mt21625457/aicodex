...
#[clap(
    ...
    // `aicodex-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `aicodex` command name that users run.
    bin_name = "aicodex",
    override_usage = "aicodex [OPTIONS] [PROMPT]\n       aicodex [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | aicodex login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv CODEX_ACCESS_TOKEN | aicodex login --with-access-token`)"
    )]
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | aicodex login --with-api-key`."
                        );
                        std::process::exit(1);
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `aicodex mcp`: `aicodex`, `aicodex exec`, `aicodex review`, `aicodex resume`, `aicodex queue`, `aicodex archive`, `aicodex delete`, `aicodex unarchive`, `aicodex fork`, `aicodex mcp`, `aicodex sandbox`, and `aicodex debug prompt-input`."
        ),
    }
}
...
/// Build the final `TuiCli` for an `aicodex resume` invocation.
...
/// Build the final `TuiCli` for an `aicodex fork` invocation.
...
mod tests {
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: aicodex plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: aicodex plugin marketplace add"),
            ("list", "Usage: aicodex plugin marketplace list"),
            ("upgrade", "Usage: aicodex plugin marketplace upgrade"),
            ("remove", "Usage: aicodex plugin marketplace remove"),
        ] {
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  aicodex resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  aicodex resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36maicodex resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          aicodex resume 123e4567-e89b-12d3-a456-426614174000
        Or run aicodex resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36maicodex resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36maicodex resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
