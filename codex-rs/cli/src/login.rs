...
//! interactive-session layers. Direct `aicodex login` intentionally does less: it preserves the
...
/// Installs a small file-backed tracing layer for direct `aicodex login` flows.
///
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    // Direct `aicodex login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `aicodex login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | aicodex login --with-api-key`.",
...
pub fn read_agent_identity_from_stdin() -> String {
    read_stdin_secret(
        "--with-agent-identity expects the Agent Identity token on stdin. Try piping it, e.g. `printenv CODEX_AGENT_IDENTITY | aicodex login --with-agent-identity`.",
...
/// `aicodex login` working in environments where device-code may be disabled/feature-gated.
