...
4. Run the installed helper path, not a project-relative script path:
   ```bash
   python "${AICODEX_HOME:-${CODEX_HOME:-$HOME/.aicodex}}/skills/.system/imagegen/scripts/remove_chroma_key.py" \
     --input <source> \
     --out <final.png> \
     --auto-key border \
     --soft-matte \
...
```
When the built-in tool returns native alpha, preserve that alpha; otherwise follow the validated chroma-key path above.
