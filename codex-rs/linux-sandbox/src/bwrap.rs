...
//! - sensitive subpaths such as `.git`, `.agents`, `.codex`, and `.aicodex` remain
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, `.codex`,
                // and `.aicodex`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".aicodex")
                {
                    return None;
                }
...
mod tests {
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".aicodex"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_aicodex = path_to_string(&temp_dir.path().join(".aicodex"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_aicodex));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_aicodex)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
