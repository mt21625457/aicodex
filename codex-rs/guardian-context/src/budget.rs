    ...
    fn add_content(mut self, item: &ContentItem) -> Self {
        match item {
            ...
            ContentItem::OutputTextWithCitations { .. } => {
                self.text_bytes = self.text_bytes.saturating_add(content_bytes(item));
            }
...
pub(super) fn content_bytes(item: &ContentItem) -> usize {
    ByteCount::measure(|counter| serde_json::to_writer(counter, item))
}

pub(super) fn content_tokens(item: &ContentItem) -> usize {
    let bytes = content_bytes(item);
