...
#[test]
fn cited_output_accounts_for_citation_payloads() {
    let content = ContentItem::OutputTextWithCitations {
        text: "answer".to_owned(),
        citations: vec![
            serde_json::from_value(serde_json::json!({
                "provider": "archive",
                "type": "url",
                "citedText": "evidence".repeat(1000),
            }))
            .unwrap(),
        ],
    };
    ...
    assert_eq!(
        SectionCost::default().add_content(&content),
        SectionCost {
            text_bytes: bytes,
            image_bytes: 0,
            image_count: 0,
    ...
    assert!(content_tokens(&content) >= 1000);
}
