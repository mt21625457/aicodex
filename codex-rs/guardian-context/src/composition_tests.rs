...
#[test]
fn cited_output_is_not_reinterpreted_as_user_input() {
    let context = ComposedContext {
        sections: vec![SectionOutput {
            id: "archived_output",
            delivery: SectionDelivery::UserContent(vec![crate::Budgeted::required(
                ContentItem::OutputTextWithCitations {
                    text: "archived answer".to_owned(),
                    citations: Vec::new(),
                },
            )]),
        }],
    ...
    assert_eq!(
        context.into_user_inputs(),
        Err(SectionError::UnsupportedDelivery {
            section: "archived_output",
        })
    );
}
