...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    let aicodex_home_env = std::env::var("AICODEX_HOME")
        .ok()
        .filter(|val| !val.is_empty());
    let codex_home_env = std::env::var("CODEX_HOME")
        .ok()
        .filter(|val| !val.is_empty());
    let home_env = aicodex_home_env
        .as_deref()
        .map(|val| ("AICODEX_HOME", val))
        .or_else(|| codex_home_env.as_deref().map(|val| ("CODEX_HOME", val)));
...
mod tests {
    ...
    fn find_codex_home_env_missing_path_is_fatal() {
        ...
        let err = find_codex_home_from_env(Some(("AICODEX_HOME", missing_str)))
            .expect_err("missing AICODEX_HOME");
        assert_eq!(err.kind(), ErrorKind::NotFound);
        assert!(
            err.to_string().contains("AICODEX_HOME"),
    ...
    fn find_codex_home_env_valid_directory_canonicalizes() {
        ...
        let resolved =
            find_codex_home_from_env(Some(("AICODEX_HOME", temp_str))).expect("valid AICODEX_HOME");
    ...
    fn find_codex_home_without_env_uses_default_home_dir() {
        let resolved =
            find_codex_home_from_env(/*codex_home_env*/ None).expect("default AICODEX_HOME");
        let mut expected = home_dir().expect("home dir");
        expected.push(".aicodex");
        let expected = AbsolutePathBuf::from_absolute_path(expected).expect("absolute home");
        assert_eq!(resolved, expected);
    }
}
