...
/// - profile   `${AICODEX_HOME}/<name>.config.toml`, when selected
...
pub async fn load_config_layers_state(
    ...
    // Add the base $AICODEX_HOME/config.toml layer. When profile-v2 is
    // selected, add the profile config as a second user layer on top so the
    // profile only needs to contain overrides.
