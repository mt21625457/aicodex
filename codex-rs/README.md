...
# Legacy aliases
aicodex debug seatbelt [--log-denials] [COMMAND]...
aicodex debug landlock [COMMAND]...
```
...
In `workspace-write`, Codex also includes `~/.aicodex/memories` in its writable roots so memory maintenance does not require an extra approval.
