...
impl ContextManager {
        ...
        T: Deref<Target = ResponseItem>,
    {
        for (item, metadata) in items {
            ...
            let is_code_mode_output = matches!(
                &processed.item,
                ResponseItem::CustomToolCallOutput { call_id, name, .. }
                    if name.as_deref() == Some(codex_code_mode::PUBLIC_TOOL_NAME)
                        || self.custom_tool_call_names_by_call_id.get(call_id).is_some_and(
                            |existing_name| existing_name == codex_code_mode::PUBLIC_TOOL_NAME
                        )
            );
            if !is_code_mode_output
    ...
    pub(crate) fn get_total_token_usage(&self, server_reasoning_included: bool) -> i64 {
        let last_tokens = self
            ...
            .unwrap_or(0);
        self.get_total_token_usage_from_last_tokens(last_tokens, server_reasoning_included)
    }

    pub(crate) fn get_total_token_usage_without_context_tokens(
        &self,
    ...
    fn get_total_token_usage_from_last_tokens(
        &self,
        last_tokens: i64,
        server_reasoning_included: bool,
    ) -> i64 {
