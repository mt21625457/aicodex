...
/// Only official OpenAI GPT models and the dedicated Codex reviewer may use Responses WebSocket.
///
...
pub(crate) fn prepare_response_items_for_request(
    ...
    if !store {
        ...
        // reasoning and apply model-aware id preservation below.
    ...
    for item in input {
        ...
        let keep = preserve_type_valid_ids
            && item.id().is_none_or(|id| match item.id_prefix() {
...
impl ModelClient {
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        self.prepare_response_items_for_request(&mut input, store, &model);
...
impl ModelClientSession {
    ...
    async fn stream_claude_messages(
        ...
        loop {
            ...
            let transport = self
                .client
                .build_api_transport(&client_setup.api_provider, CLAUDE_MESSAGES_ENDPOINT)?;
            let request_auth_context = AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(CodexAuth::auth_mode),
                client_setup.api_auth.as_ref(),
                client_setup.agent_identity_telemetry.clone(),
    ...
    pub async fn count_claude_context_tokens(
        ...
        let transport = self
            .client
            .build_api_transport(&client_setup.api_provider, CLAUDE_COUNT_TOKENS_ENDPOINT)?;
        let request_auth_context = AuthRequestTelemetryContext::new(
            client_setup.auth.as_ref().map(CodexAuth::auth_mode),
            client_setup.api_auth.as_ref(),
            client_setup.agent_identity_telemetry.clone(),
