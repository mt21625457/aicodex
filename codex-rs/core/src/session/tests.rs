...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    let (session, _turn_context) = make_session_and_context().await;
    {
        let mut state = session.state.lock().await;
        let mut config = (*state.session_configuration.original_config_do_not_use).clone();
        config.model = Some("session-static-model".to_string());
        state.session_configuration.original_config_do_not_use = Arc::new(config);
    }
    let codex_home = session.codex_home().await;
    std::fs::create_dir_all(&codex_home).expect("create codex home");
...
"#,
    ...
    let config = session.get_config().await;
    assert_eq!(config.model.as_deref(), Some("session-static-model"));
    let apps_toml = config
        .config_layer_stack
        .effective_config()
        .as_table()
...
async fn session_configuration_provider_update_changes_snapshot_and_turn_config() {
    ...
    alternate_provider.wire_api = WireApi::Responses;

    let err = match session_configuration.apply(
        &SessionSettingsUpdate {
    ...
    assert_eq!(
        updated.thread_config_snapshot(Vec::new()).model_provider_id,
        "alternate_provider"
    );
    assert_eq!(updated.provider_id, "alternate_provider");
    assert_eq!(updated.provider.info(), &alternate_provider);

    let mut per_turn_config = (*updated.original_config_do_not_use).clone();
    per_turn_config.model_provider_id = updated.provider_id;
    per_turn_config.model_provider = alternate_provider.clone();
    ...
    assert_eq!(
        session_configuration
            .thread_config_snapshot(Vec::new())
            .model_provider_id,
...
async fn mcp_elicitation_reviewer_uses_active_reviewer_and_latest_runtime_policy(
    ...
    .to_string();
    let _guardian_request = mount_sse_once(
        &guardian_server,
        sse(vec![
            ev_response_created("resp-guardian-mcp-elicitation"),
            ev_assistant_message("msg-guardian-mcp-elicitation", &assessment),
            ev_completed("resp-guardian-mcp-elicitation"),
        ]),
    )
    .await;
...
async fn refresh_mcp_servers_preserves_elicitation_auto_deny() -> anyhow::Result<()> {
    let (session, turn_context) = make_session_and_context().await;
    let session = Arc::new(session);
    let runtime = Arc::clone(&session.services.mcp_runtime);
    runtime.set_elicitations_auto_deny(/*auto_deny*/ true);
    ...
    assert!(runtime.elicitations_auto_deny());
...
async fn deferred_environment_roots_refresh_plugin_availability() {
                ...
                available
                    .then(
                        || codex_extension_api::McpServerContribution::SelectedPluginPackage {
                            selected_root_id: "skill-only".to_string(),
    ...
    let originator = session.originator().await;
    let projection = session
        ...
        .runtime_config_for_step(
            ...
            crate::mcp::McpThreadIdentity {
                ...
                environments: crate::mcp::McpEnvironmentScope::Live(
                    &session.services.turn_environments,
                ),
            },
        ...
        .await;
    assert!(projection.plugins_available);

    let desired = session
        .latest_mcp_desired_state(session.services.auth_manager.auth_cached())
        .await;
    session
        .publish_mcp_runtime(
            &desired,
        ...
        .await;
    let new_binding = session
        .services
        .mcp_runtime
        .current_binding()
        .await
        .expect("refreshed MCP binding");

    assert!(!old_binding.plugins_available());
    assert!(new_binding.plugins_available());
    assert!(!Arc::ptr_eq(&old_binding, &new_binding));
}
...
async fn refresh_mcp_servers_shuts_down_replaced_stdio_server_process() -> anyhow::Result<()> {
    ...
    refresh_config
        .mcp_servers
        .set(HashMap::from([("leaky_stdio".to_string(), server_config)]))?;
    session
        .refresh_mcp_servers_now(
            &turn_context,
            &refresh_config,
    ...
    let mut refresh_config = turn_context.config.as_ref().clone();
    refresh_config.mcp_servers.set(HashMap::new())?;
    session
        .refresh_mcp_servers_now(
            &turn_context,
            &refresh_config,
...
async fn build_initial_context_orders_multi_agent_mode_after_other_agent_policy() {
    ...
    update_turn_settings_for_test(&mut turn_context, |settings| {
        update_selected_settings_for_test(settings, |selected| {
            selected.collaboration_mode.settings.developer_instructions =
