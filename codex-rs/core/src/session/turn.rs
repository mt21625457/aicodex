...
pub(crate) async fn run_turn(
    ...
    input: Vec<TurnInput>,
    empty_input_policy: EmptyInputTurnPolicy,
    ...
    // Pre-turn compaction runs before context updates and the new user message are recorded.
    let initial_pre_sampling_compacted = match run_initial_pre_sampling_compact(
        &sess,
        &turn_context,
        &mut client_session,
        &cancellation_token,
    ...
    let mut post_compaction_model_follow_up_active = initial_pre_sampling_compacted;
    ...
    loop {
            ...
            && pre_sampling_admission_compactions < MAX_PRE_SAMPLING_ADMISSION_COMPACTIONS_PER_TURN
        {
            match Box::pin(run_pre_sampling_admission_compact(
                ...
                &turn_context,
                &mut client_session,
                /*allow_estimated_context_admission*/ true,
                &cancellation_token,
            ))
            .await
            {
                Ok(true) => {
                    ...
                    post_compaction_model_follow_up_active = true;
                    continue;
                }
                Ok(false) => {}
                Err(err) => {
        ...
        } else if should_run_pre_sampling_admission {
            warn!(
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                let requested_new_context_window = sess.take_new_context_window_request().await;
                let should_defer_compaction_to_drain_pending_input =
                    pending_input_deferred_for_model_follow_up
                        && !model_needs_follow_up
                        && has_pending_input;
                let should_roll_over = needs_follow_up
                    && (requested_new_context_window
                        || (token_limit_reached
                            && !should_defer_compaction_to_drain_pending_input));
                ...
                if should_defer_compaction_to_drain_pending_input || !has_pending_input {
                    pending_input_deferred_for_model_follow_up = false;
                }
...
#[instrument(level = "trace", skip_all)]
async fn run_initial_pre_sampling_compact(
    sess: &Arc<Session>,
    turn_context: &Arc<TurnContext>,
...
async fn pre_sampling_admission_status(
    sess: &Session,
    turn_context: &TurnContext,
    allow_estimated_context_admission: bool,
) -> PreSamplingAdmissionStatus {
    let token_status = super::context_window::context_window_token_status(sess, turn_context).await;
    let estimated_context_tokens = if allow_estimated_context_admission {
        sess.get_estimated_context_token_count().await
    } else {
        None
    };
    // Chat and Responses already project locally appended items onto their recorded usage in
    // `ContextWindowTokenStatus`. Reapplying the full local estimate would double-count the fixed
    // prefix and reasoning that the provider has already accounted for. Claude's native admission
    // path still needs the full estimate before count-tokens has established a baseline.
    let estimated_auto_compact_scope_tokens = (allow_estimated_context_admission
        && turn_context.provider.info().wire_api == WireApi::Claude)
        .then_some(estimated_context_tokens)
        .flatten()
        .map(
            |tokens| match turn_context.config.model_auto_compact_token_limit_scope {
...
async fn run_pre_sampling_admission_compact(
    ...
    let admission_status = pre_sampling_admission_status(
        sess.as_ref(),
        turn_context.as_ref(),
        allow_estimated_context_admission,
    )
    .await;
...
pub(crate) fn build_prompt(
    ...
    Prompt {
        ...
        hidden_tools: step_context.tool_router.hidden_specs(),
...
async fn update_responses_token_usage_context_estimate(
    sess: &Session,
    turn_context: &TurnContext,
    streamed_token_usage: Option<&TokenUsage>,
) {
    let Some(estimated_context_tokens) = sess.get_estimated_context_token_count().await else {
        return;
    };
    let provider_context_tokens = streamed_token_usage.map(|usage| {
        usage
            .total_tokens
            .max(usage.input_tokens.saturating_add(usage.output_tokens))
    });
    let context_tokens = provider_context_tokens
        .unwrap_or_default()
    ...
    let token_info = {
        ...
        let mut info = state
            .token_info()
            .unwrap_or_else(|| TokenUsageInfo::empty(turn_context.model_context_window()));
        if provider_context_tokens.is_some() {
            // Keep the provider's detailed last-response usage intact. The local value is a
            // supplemental estimate of current context occupancy, not replacement response usage.
            info.context_tokens = Some(context_tokens.max(0));
...
async fn try_run_sampling_request(
    ...
    let mut completed_responses_token_usage: Option<TokenUsage> = None;
    ...
    if should_refresh_claude_context_usage {
        ...
        if turn_context.provider.info().wire_api == WireApi::Responses {
            update_responses_token_usage_context_estimate(
                sess.as_ref(),
                turn_context.as_ref(),
                completed_responses_token_usage.as_ref(),
            )
            .await;
        }
        sess.send_token_count_event(&turn_context).await;
