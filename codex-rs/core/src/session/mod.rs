...
impl Session {
    ...
    pub(crate) async fn get_total_token_usage_without_context_tokens(&self) -> i64 {
        let state = self.state.lock().await;
        state
            .history
            .get_total_token_usage_without_context_tokens(state.server_reasoning_included())
    }
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        let turn_context = match conversation_history {
            ...
            InitialHistory::Forked(mut rollout_items) => {
                ...
                self.persist_rollout_items_with_mode(
                    &rollout_items,
    ...
    pub(super) fn thread_metadata_patch_for_configuration_update(
        ...
        let model_changed = previous.step_settings.collaboration_mode.model()
            != updated.step_settings.collaboration_mode.model();
        let effort_changed = previous.step_settings.collaboration_mode.reasoning_effort()
            != updated.step_settings.collaboration_mode.reasoning_effort();

        (provider_changed || model_changed || effort_changed).then(|| ThreadMetadataPatch {
            model_provider: Some(updated.provider_id.clone()),
            model: Some(updated.step_settings.collaboration_mode.model().to_string()),
            reasoning_effort: Some(updated.step_settings.collaboration_mode.reasoning_effort()),
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        self.send_event_with_persistence_mode(turn_context, msg, EventPersistenceMode::Limited)
            .await;
    }

    /// Persist the event to rollout using `mode` and send it to clients.
    pub(crate) async fn send_event_with_persistence_mode(
        &self,
        turn_context: &TurnContext,
        msg: EventMsg,
        ...
        self.send_event_raw_with_persistence_mode(event, mode).await;
        ...
        for legacy in legacy_source.as_legacy_events(show_raw_agent_reasoning) {
            ...
            self.send_event_raw_with_persistence_mode(legacy_event, mode)
                .await;
        }
    }
    ...
    pub(crate) async fn send_event_raw(&self, event: Event) {
        self.send_event_raw_with_persistence_mode(event, EventPersistenceMode::Limited)
            .await;
    }

    pub(crate) async fn send_event_raw_with_persistence_mode(
        &self,
        event: Event,
        mode: EventPersistenceMode,
    ) {
        self.send_event_raw_with_persistence(event, /*persist*/ true, mode)
            .await;
    }
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        ...
        self.send_event_raw_with_persistence(event, persist, EventPersistenceMode::Limited)
            .await;
    }
    ...
    async fn send_event_raw_with_persistence(
        ...
        if persist {
            ...
            self.persist_rollout_items_with_mode(&rollout_items, mode)
                .await;
        }
        self.services
            .rollout_thread_trace
                ...
                .map(|content| {
                    ContentItemKind(
                        match content {
                            ...
                            | ContentItem::OutputTextWithCitations { .. } => "user.text",
    ...
    pub(crate) async fn persist_rollout_items(&self, items: &[RolloutItem]) {
        self.persist_rollout_items_with_mode(items, EventPersistenceMode::Limited)
            .await;
    }

    pub(crate) async fn persist_rollout_items_with_mode(
        &self,
        items: &[RolloutItem],
        mode: EventPersistenceMode,
    ) {
        if let Some(live_thread) = self.live_thread()
            && let Err(e) = live_thread
                .append_items_with_persistence_mode(items, mode)
                .await
        {
    ...
    pub(crate) async fn update_token_usage_info_with_context_count(
        ...
        self.notify_token_usage_contributors(turn_context, token_info.as_ref())
            .await;
        self.send_token_count_event(turn_context).await;
    }
    ...
    pub(crate) async fn notify_token_usage_contributors(
        &self,
        turn_context: &TurnContext,
