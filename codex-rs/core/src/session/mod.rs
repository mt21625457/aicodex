...
impl Session {
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        match conversation_history {
            ...
            InitialHistory::Forked(mut rollout_items) => {
                ...
                // If persisting, copy replayable fork history into the new rollout.
                // Paginated subagents already persisted their inherited model context while
                // creating the live thread, so do not append that prefix a second time.
    ...
    pub(crate) async fn update_settings(
        ...
        let (previous_config, new_config, permission_profile_changed, metadata_patch) = {
            let mut state = self.state.lock().await;
    ...
    pub(super) fn thread_metadata_patch_for_configuration_update(
        ...
        (provider_changed || model_changed || effort_changed).then(|| ThreadMetadataPatch {
            ...
            reasoning_effort: Some(updated.collaboration_mode.reasoning_effort()),
