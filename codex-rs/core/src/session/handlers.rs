...
pub(super) async fn user_input_or_turn_inner(
    ...
    let (
        ...
        Op::UserTurn {
            ...
            let legacy_fallback_cwd = {
                let state = sess.state.lock().await;
                AbsolutePathBuf::resolve_path_against_base(&cwd, state.session_configuration.cwd())
            };
            let environments = Some(TurnEnvironmentSelections::new(
                legacy_fallback_cwd,
    ...
    let mut updates = if apply_thread_settings {
