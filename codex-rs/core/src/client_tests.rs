...
fn store_false_drops_id_only_official_reasoning_shells() {
    ...
    assert!(
        third_party.iter().all(|item| item.id().is_none()),
        "compatible providers must not receive unstored item IDs"
    );
    ...
    assert_eq!(
        official[1].id().map(ResponseItemId::as_str),
        Some("msg_assistant")
    );
}
...
fn new_session_for_provider_uses_turn_provider_without_mutating_default_client() {
    ...
    let alternate_session = client.new_session_for_provider(claude_provider);
    assert_eq!(
        alternate_session.provider_info().wire_api,
        WireApi::Responses
    );
    assert_eq!(
        alternate_session.provider_info().base_url.as_deref(),
        Some("https://alternate.example.com/v1")
    );
