...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&cwd.path().join(".aicodex"), cwd.path()),
            "expected :workspace metadata carveouts, policy: {policy:?}"
        );
    }
    Ok(())
...
#[tokio::test]
async fn sqlite_home_honors_env_or_defaults_to_codex_home_for_workspace_write()
-> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
    ...
    let expected_sqlite_home = std::env::var(codex_state::SQLITE_HOME_ENV)
        .ok()
    ...
    assert_eq!(config.sqlite.home(), expected_sqlite_home.as_path());
