...
pub struct Config {
    ...
    /// file: AICODEX_HOME/.credentials.json
...
impl Config {
    ...
    /// ignores user settings. For example, the `aicodex exec` subcommand is
...
/// Patch `AICODEX_HOME/config.toml` project state to set trust level.
/// Use with caution.
