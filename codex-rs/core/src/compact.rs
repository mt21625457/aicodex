...
async fn run_compact_task_inner_impl(
    ...
    let compacted_source_items = history_items
        .split_last()
        .map(|(_compact_prompt, source_items)| source_items)
        .unwrap_or_default();
    let summary_suffix = get_last_assistant_message_from_turn(&output_items).unwrap_or_default();
    let summary_text = format!("{SUMMARY_PREFIX}\n{summary_suffix}");
    let user_messages = collect_user_messages(compacted_source_items);
