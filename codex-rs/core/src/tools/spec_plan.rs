...
fn build_code_mode_executors(
    ...
    let code_mode_exec_prompt_tool_specs = executors
        ...
        .collect::<Vec<_>>();
    let namespace_descriptions =
        code_mode_namespace_descriptions(&code_mode_exec_prompt_tool_specs);
...
fn add_tool_sources(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    add_hosted_model_tools(context, planned_tools);
}
...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    if environment_mode.has_environment() && apply_patch_tool_type.is_some() {
        ...
        planned_tools.add_runtime(ClaudeTextEditorHandler::new(include_environment_id));
    }
...
fn add_hosted_model_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    for spec in hosted_model_tool_specs(context.turn_context) {
        if matches!(spec, ToolSpec::WebSearch { .. }) {
            planned_tools.add_runtime(WebSearchHandler::new(spec.clone()));
        }
        planned_tools.hosted_specs.push(spec);
    }
}
