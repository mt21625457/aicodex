...
pub(crate) fn finalize_tool_router(
    ...
    // Hosted web-search specs are model-visible, but compatible Claude providers issue them as
    // local function calls. Keep a hidden runtime so those calls share the normal tool pipeline.
...
fn add_collaboration_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if collab_tools_enabled(turn_context, context.model_info) {
        if multi_agent_v2_enabled(turn_context) {
            ...
            let is_grok = is_grok_model_slug(&context.model_info.slug);
            ...
            let agent_type_description =
                agent_type_description(turn_context, context.default_agent_type_description);
            let is_grok = is_grok_model_slug(&context.model_info.slug);
            let exposure = if search_tool_enabled(turn_context, context.model_info) && !is_grok {
                ToolExposure::Deferred
            } else {
                ToolExposure::Direct
            };
            let tool_namespace = (!is_grok).then_some(MULTI_AGENT_V1_NAMESPACE);
            registry.register_trusted_with_exposure(
            ...
            registry.register_trusted_with_exposure(
                multi_agent_v1_handler(SendInputHandler, tool_namespace),
                exposure,
            );
            registry.register_trusted_with_exposure(
                multi_agent_v1_handler(ResumeAgentHandler, tool_namespace),
                exposure,
            );
            registry.register_trusted_with_exposure(
                multi_agent_v1_handler(
            ...
            registry.register_trusted_with_exposure(
                multi_agent_v1_handler(CloseAgentHandler, tool_namespace),
                exposure,
            );
        }
    }
...
fn multi_agent_v1_handler(
    ...
    match namespace {
        Some(_) => Arc::new(handler),
        None => Arc::new(MultiAgentV1PlainOverride {
            handler: Arc::new(handler),
        }),
    }
}

struct MultiAgentV1PlainOverride {
    handler: Arc<dyn CoreToolRuntime>,
}

impl ToolExecutor<ToolInvocation> for MultiAgentV1PlainOverride {
    fn tool_name(&self) -> ToolName {
        ToolName::plain(self.handler.tool_name().name)
    }

    fn spec(&self) -> ToolSpec {
        let ToolSpec::Namespace(namespace) = self.handler.spec() else {
            unreachable!("multi-agent v1 tools use namespace specs");
        };
        let [tool] = namespace.tools.as_slice() else {
            unreachable!("each multi-agent v1 runtime owns one tool");
        };
        match tool {
            ResponsesApiNamespaceTool::Function(tool) => ToolSpec::Function(tool.clone()),
            ResponsesApiNamespaceTool::Custom(tool) => ToolSpec::Freeform(tool.clone()),
        }
    }
...
impl CoreToolRuntime for MultiAgentV1PlainOverride {
