...
#[test]
fn spawn_agent_service_tier_inheritance_uses_root_preference_and_child_model() {
    std::thread::Builder::new()
        .name(
            "spawn_agent_service_tier_inheritance_uses_root_preference_and_child_model".to_string(),
        )
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .block_on(Box::pin(
                    spawn_agent_service_tier_inheritance_uses_root_preference_and_child_model_inner(
                    ),
                ));
        })
...
async fn spawn_agent_service_tier_inheritance_uses_root_preference_and_child_model_inner() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }
...
#[test]
fn multi_agent_v2_full_history_fork_inherits_root_service_tier() {
    std::thread::Builder::new()
        .name("multi_agent_v2_full_history_fork_inherits_root_service_tier".to_string())
        .stack_size(/*size*/ 8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_full_history_fork_inherits_root_service_tier_inner(),
                ));
        })
        .expect("large-stack test thread should spawn")
        .join()
...
async fn multi_agent_v2_full_history_fork_inherits_root_service_tier_inner() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        task_name: String,
    }
...
#[test]
fn multi_agent_v2_list_agents_returns_completed_status_without_encrypted_spawn_preview() {
    std::thread::Builder::new()
        .name(
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_list_agents_returns_completed_status_without_encrypted_spawn_preview_inner(),
                ));
        })
        .expect("large-stack test thread should spawn")
        .join()
...
async fn multi_agent_v2_list_agents_returns_completed_status_without_encrypted_spawn_preview_inner()
{
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let root = manager
...
fn multi_agent_v2_list_agents_filters_by_relative_path_prefix() {
    std::thread::Builder::new()
        .name("multi_agent_v2_list_agents_filters_by_relative_path_prefix".to_string())
        .stack_size(/*size*/ 8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_list_agents_filters_by_relative_path_prefix_inner(),
                ));
        })
        .expect("large-stack test thread should spawn")
        .join()
...
async fn multi_agent_v2_list_agents_filters_by_relative_path_prefix_inner() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let mut config = (*turn.config).clone();
    let _ = config.features.enable(Feature::MultiAgentV2);
...
fn multi_agent_v2_list_agents_omits_closed_agents() {
    std::thread::Builder::new()
        .name("multi_agent_v2_list_agents_omits_closed_agents".to_string())
        .stack_size(/*size*/ 8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_list_agents_omits_closed_agents_inner(),
                ));
        })
        .expect("large-stack test thread should spawn")
        .join()
...
async fn multi_agent_v2_list_agents_omits_closed_agents_inner() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let root = manager
        .start_thread(StartThreadOptions::new((*turn.config).clone()))
...
fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    std::thread::Builder::new()
        .name("multi_agent_v2_interrupted_turn_does_not_notify_parent".to_string())
        .stack_size(/*size*/ 8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_interrupted_turn_does_not_notify_parent_inner(),
                ));
        })
        .expect("large-stack test thread should spawn")
        .join()
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent_inner() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let root = manager
        .start_thread(StartThreadOptions::new((*turn.config).clone()))
