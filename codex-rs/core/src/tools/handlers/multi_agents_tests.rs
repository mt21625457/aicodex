...
fn spawn_agent_service_tier_inheritance_preserves_supported_or_configured_tiers() {
    std::thread::Builder::new()
        .name("spawn_agent_service_tier_inheritance_preserves_supported_or_configured_tiers".to_string())
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .block_on(Box::pin(
                    spawn_agent_service_tier_inheritance_preserves_supported_or_configured_tiers_inner(),
                ));
        })
...
async fn spawn_agent_service_tier_inheritance_preserves_supported_or_configured_tiers_inner() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }
...
fn multi_agent_v2_full_history_fork_accepts_explicit_service_tier() {
    std::thread::Builder::new()
        .name("multi_agent_v2_full_history_fork_accepts_explicit_service_tier".to_string())
        .stack_size(/*size*/ 8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_full_history_fork_accepts_explicit_service_tier_inner(),
                ));
        })
        .expect("large-stack test thread should spawn")
        .join()
...
async fn multi_agent_v2_full_history_fork_accepts_explicit_service_tier_inner() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        task_name: String,
    }
