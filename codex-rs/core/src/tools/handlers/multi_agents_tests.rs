...
fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    std::thread::Builder::new()
        .name(
            "multi_agent_v2_list_agents_returns_completed_status_and_last_task_message".to_string(),
        )
        .stack_size(/*size*/ 8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .expect("test runtime should build")
                .block_on(Box::pin(
                    multi_agent_v2_list_agents_returns_completed_status_and_last_task_message_inner(
                    ),
                ));
        })
        .expect("large-stack test thread should spawn")
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message_inner() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let root = manager
        .start_thread((*turn.config).clone())
