...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "gpt-5.1-codex-test";
...
impl TestCodexBuilder {
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = codex_utils_cargo_bin::cargo_bin("aicodex") {
...
fn ensure_test_model_catalog(config: &mut Config) -> Result<()> {
    let Some(model_slug) = config.model.as_deref().filter(|model| {
        matches!(
            *model,
            TEST_MODEL_WITH_EXPERIMENTAL_TOOLS | "test-gpt-5.1-codex"
        )
    }) else {
        return Ok(());
    };
