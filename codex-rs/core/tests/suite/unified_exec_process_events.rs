...
async fn serve_exec_with_pushed_events(
    ...
    let process_start = loop {
        ...
        match request["method"].as_str() {
            ...
            Some("fs/conditionalWriteFile") if unrestricted_patch => {
...
"#,
    ...
    if matches!(
        ...
        let write_request = exec_server_result.process_start;
        assert_eq!(write_request["method"], "fs/conditionalWriteFile");
        assert_eq!(
            write_request["params"]["path"],
        ...
        assert_eq!(
            write_request["params"]["precondition"],
            json!({"type": "mustNotExist"})
        );
        assert_eq!(
            BASE64_STANDARD.decode(
                write_request["params"]["dataBase64"]
