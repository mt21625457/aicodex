...
"#,
    ...
    let emitted_variable = variable_output(output, /*truncated*/ false);
    assert!(emitted_variable.len() < 50_000);
    assert!(emitted_variable.contains("tokens truncated"));
    assert!(emitted_variable.starts_with('x') && emitted_variable.ends_with('x'));
...
const [head, tail] = result.output.split("…2500 tokens truncated…");
text(JSON.stringify({
  truncated: tail !== undefined,
  headLength: head.split("\n\n")[1].length,
...
"#,
    ...
    assert_eq!(
        serde_json::from_str::<Value>(output)?,
        serde_json::json!({"truncated": true, "headLength": 40_000, "tailLength": 40_000})
    );
