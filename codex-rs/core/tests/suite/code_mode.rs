...
fn assert_repeated_output(output: &str, byte: u8, expected_len: usize) {
    assert_eq!(output.len(), expected_len);
    assert_eq!(
        output.as_bytes().iter().position(|actual| *actual != byte),
        None,
        "output should only contain byte {byte:?}"
    );
}

fn assert_middle_truncated_repeated_output(
    output: &str,
    byte: u8,
    head_len: usize,
    marker: &str,
    ...
    let output = if let Some(rest) =
        output.strip_prefix("Warning: truncated output (original token count: ")
    {
        let (_token_count, rest) = rest
            .split_once(")\n")
            .expect("truncated output warning should include token count");
        rest
    } else {
        output
    };
    let body = output
        ...
        .expect("truncated output should include line-count header");
    let (head, tail) = body
        .split_once(marker)
        .expect("truncated output should include marker");
    assert_repeated_output(head, byte, head_len);
    assert_repeated_output(tail, byte, tail_len);
}

fn variable_output(output: &str, truncated: bool) -> &str {
    let flag = if truncated { "True" } else { "False" };
    let prefix = format!("Variable truncated: {flag}. Variable: ");
    output
        .strip_prefix(&prefix)
        .expect("output should include variable truncation status")
}
...
async fn run_code_mode_turn_with_config(
    ...
    run_code_mode_turn_with_model_and_config(server, prompt, code, "gpt-5.1-codex-test", configure)
        .await
}
...
"#,
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("gpt-5.1-codex-test")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "gpt-5.1-codex-test").await
}
...
"#,
    ...
    let tokens_left = parsed
        ...
        .expect("code-mode get_context_remaining result should include tokens_left");
    assert!(tokens_left > 0);
    assert!(tokens_left < 9_500);
    assert_eq!(parsed, serde_json::json!({ "tokens_left": tokens_left }));
...
"#,
    ...
    assert_repeated_output(variable_output(output, /*truncated*/ false), b'x', 50_000);
...
"#,
    ...
    let items = custom_tool_output_items(&request, "call-1");
    let output = text_item(&items, /*index*/ 1);
    assert_middle_truncated_repeated_output(
        variable_output(output, /*truncated*/ true),
        b'A',
        40_000,
...
async fn code_mode_node_repl_text_evidence_is_visible_only_to_guardian(
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if reviewer_token_budget {
                    ...
                    reviewer.context_window =
                        (reviewer_constraint == Some("small")).then_some(15_000);
    ...
    let snapshot_padding = if reviewer_constraint == Some("small") {
        4_000
