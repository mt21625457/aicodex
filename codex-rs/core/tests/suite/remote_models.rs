...
async fn unknown_model_sends_builtin_instructions() -> Result<()> {
    ...
    let response = mount_sse_once(
        &server,
        sse(vec![
            ev_response_created("resp-unknown"),
            ev_completed("resp-unknown"),
        ]),
    )
    .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn grok_remote_model_exposes_plain_multi_agent_v2_tools() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));
    ...
    for tool_name in [
        ...
        assert!(
            ...
            "expected top-level function tool {tool_name}, got {tools:?}"
        );
    }
    assert!(
        !tools
            .iter()
            .any(|tool| tool["type"] == "namespace" && tool["name"] == "collaboration"),
        "Grok must not receive the unsupported collaboration namespace: {tools:?}"
    );
