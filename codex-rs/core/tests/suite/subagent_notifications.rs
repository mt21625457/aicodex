        ...
        |req: &wiremock::Request| {
            body_contains(req, CHILD_PROMPT)
                ...
                && decoded_body(req)
                    ...
                    .is_some_and(|metadata| metadata["agent_name"] == "/root/worker")
        },
        sse(vec![
            ev_response_created("resp-child-1"),
            ev_assistant_message("msg-child-1", "child done"),
...
async fn spawned_multi_agent_v2_child_inherits_parent_developer_context() -> Result<()> {
    ...
    let child_request = loop {
        ...
        if Instant::now() >= deadline {
            anyhow::bail!("timed out waiting for the /root/worker request");
        }
        sleep(Duration::from_millis(10)).await;
    };
    ...
    let parent_request = parent_request_log
        ...
        .find(|request| {
            request.body_json()["client_metadata"]["x-codex-turn-metadata"]
                .as_str()
                .and_then(|metadata| serde_json::from_str::<Value>(metadata).ok())
                .is_some_and(|metadata| metadata["agent_name"] == "/root")
        })
        .expect("parent request log should capture the /root request");
...
async fn plaintext_multi_agent_v2_completion_sends_agent_message(
    ...
    .await;
    let error = "stream disconnected before completion: closed_before_message_start: stream closed before response.completed";
