...
impl App {
    ...
    // Construct the large event-loop future in a separate stack frame. Boxing it at the
    // caller still reserves a full temporary in the caller throughout every poll.
    #[inline(never)]
    ...
    pub fn run(
        ...
        startup_draft: StartupDraftPump,
        managed_worktree: Option<crate::ManagedTuiWorktree>,
    ) -> std::pin::Pin<Box<impl std::future::Future<Output = Result<AppExitInfo>> + '_>> {
