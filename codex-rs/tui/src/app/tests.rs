...
fn selected_and_resumed_v1_and_v2_children_accept_direct_input() -> Result<()> {
    ...
    run_async_test_with_stack(
        ...
        || async {
            ...
            let root_timestamp = "2026-01-01T00:00:00Z";
            let root_meta = SessionMeta {
                session_id: root_thread_id.into(),
                id: root_thread_id,
                timestamp: root_timestamp.to_string(),
            ...
            let root_meta_line = serde_json::json!({
                "timestamp": root_timestamp,
                "type": "session_meta",
                "payload": serde_json::to_value(root_meta)?,
            });
            std::fs::write(root_rollout_path, format!("{root_meta_line}\n"))?;
            ...
            // Start after writing the fixture rollouts so the state-db migration discovers the
            // same persisted parent/child topology that a real process restart would see.
            let mut app_server =
                crate::start_embedded_app_server_for_picker(app.chat_widget.config_ref()).await?;
            let root = app_server
                .resume_thread(
...
#[tokio::test]
async fn prompt_edit_forks_before_selected_prompt_preserves_source_and_discards_source_events()
-> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let config = app.chat_widget.config_ref().clone();
    ...
    app.enqueue_primary_thread_session(started.session, started.turns)
        .await?;
    let source_event_tx = app
        .thread_event_channels
        .get(&source_thread_id)
        .expect("source thread channel")
        .sender
        .clone();
    source_event_tx
        .send(ThreadBufferedEvent::Notification(Box::new(
            ServerNotification::Warning(WarningNotification {
                ...
                message: "stale source warning".to_string(),
            }),
        )))
        .await
        .expect("source event should queue");
    {
        let mut store = app
            .thread_event_channels
            .get(&source_thread_id)
    ...
    assert!(source_event_tx.is_closed());
