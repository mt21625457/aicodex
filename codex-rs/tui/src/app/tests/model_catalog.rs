...
fn model_presets_with_test_upgrades() -> Vec<ModelPreset> {
    ...
    presets.retain(|existing| existing.model != preset.model);
    presets.push(preset);
    presets
}
...
fn retired_model_migration_respects_catalog_metadata() {
    ...
    let current = presets
        ...
        .expect("test preset");
    current.id = "retired-test-model".to_string();
    current.model = "retired-test-model".to_string();
    let expected = current.upgrade.clone();
    assert_eq!(
        model_upgrade_for_migration("retired-test-model", &presets),
        expected
    );

    presets
        .iter_mut()
        .find(|preset| preset.model == "retired-test-model")
