...
impl StartupDraft {
    ...
    pub(crate) fn new(
        ...
        let mut draft = Self {
            ...
            pump: StartupDraftPump {
                header: startup_session_header(/*config*/ None, CODEX_CLI_VERSION),
                bottom_pane,
                events,
                app_event_rx,
                initial_screen,
...
impl StartupDraftPump {
    ...
    pub(crate) fn apply_config(&mut self, config: &Config) {
        self.header = startup_session_header(Some(config), CODEX_CLI_VERSION);
        self.bottom_pane
...
fn startup_session_header(config: Option<&Config>, version: &'static str) -> Box<dyn HistoryCell> {
