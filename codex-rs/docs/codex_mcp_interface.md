...
- Server binary: `aicodex mcp-server` (or `codex-mcp-server`)
...
```bash
aicodex mcp-server | your_mcp_client
```
...
Use the separate `aicodex mcp` subcommand to manage configured MCP server launchers in `config.toml`.
