...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_read(
        ...
        let paginated_full_history = include_turns
            && matches!(
        ...
        self.outgoing.send_response(request_id, response).await;
        if paginated_full_history {
            self.send_deprecation_notice(connection_id, PAGINATED_THREAD_READ_DEPRECATION_SUMMARY)
                .await;
        }
    ...
    async fn read_thread_view(
        ...
        } else if let Some((thread, rollout_items)) = self
            .load_persisted_thread_for_read(thread_id, include_turns, items_view)
            .await?
        {
            if let Some(loaded_thread) = loaded_thread.as_ref() {
...
fn reconstruct_thread_turns_for_turns_list(
    ...
    if let Some(active_turn) = active_turn {
        ...
        apply_thread_turns_items_view(&mut turns, items_view);
    }
    turns
}
