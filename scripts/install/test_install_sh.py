...
class InstallShTest(unittest.TestCase):
    def test_metadata_fetch_failure_is_not_reported_as_missing_assets(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/leagsoft/aicodex/releases/tags/"
                f"rust-v{VERSION}"
            ],
        )
        self.assertIn(
            f"Could not fetch GitHub release metadata for AICodex {VERSION}",
            result.stderr,
        )
        self.assertNotIn("Could not find AICodex package", result.stderr)
    ...
    def test_exact_release_opt_out_uses_github_metadata_once(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "https://api.github.com/repos/leagsoft/aicodex/releases/tags/"
                f"rust-v{VERSION}",
                "https://github.com/leagsoft/aicodex/releases/download/"
    ...
    def test_latest_release_reuses_version_metadata(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/leagsoft/aicodex/releases/latest",
                "https://github.com/leagsoft/aicodex/releases/download/"
                f"rust-v{VERSION}/codex-package_SHA256SUMS",
            ],
        )
    ...
    def test_macos_install_exposes_code_mode_host_beside_aicodex(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            aicodex_path = install_bin / "aicodex"
            host_path = install_bin / "codex-code-mode-host"
            self.assertEqual(
                os.readlink(aicodex_path), str(current / "bin" / "aicodex")
            )
            self.assertEqual(
    ...
    def test_releases_corrupt_assets_fall_back_to_github(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("checksum did not match release metadata", result.stderr)
            self.assertIn("retrying from GitHub Releases", result.stderr)
    ...
    def test_releases_exact_legacy_fallback_reuses_offline_install(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotIn("Downloading AICodex CLI", second_result.stdout)
        ...
        textwrap.dedent(
            ...
            case "$url" in
              ...
              https://releases.openai.com/codex/releases/*/codex-package-*.tar.gz)
                ...
                ;;
              https://github.com/leagsoft/aicodex/releases/download/*/codex-package_SHA256SUMS)
                ...
                ;;
              https://github.com/leagsoft/aicodex/releases/download/*/codex-package-*.tar.gz)
                if [ -n "$CODEX_TEST_ARCHIVE_PATH" ]; then
                  cp "$CODEX_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                ...
                ;;
              https://github.com/leagsoft/aicodex/releases/download/*/codex-npm-*.tgz)
...
def create_package_release(
    ...
    *,
    ...
    write_executable(
        package_dir / "bin" / "aicodex",
        f"#!/bin/sh\nprintf 'aicodex-cli {VERSION}\\n'\n",
    )
    write_executable(
...
def create_legacy_release(root: Path) -> tuple[Path, str]:
    ...
    (vendor_dir / "aicodex").mkdir(parents=True)
    (vendor_dir / "path").mkdir()
    write_executable(
        vendor_dir / "aicodex" / "aicodex",
        f"#!/bin/sh\nprintf 'aicodex-cli {VERSION}\\n'\n",
    )
