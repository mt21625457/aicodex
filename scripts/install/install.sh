...
BIN_PATH="$BIN_DIR/aicodex"
CODEX_HOME_DIR="${AICODEX_HOME:-${CODEX_HOME:-$HOME/.aicodex}}"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-alpha(\.[0-9]+){0,2}|-beta(\.[0-9]+)?)?$'; then
    echo "Invalid AICodex release version: $version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]." >&2
    exit 1
  fi
}
...
download_file() {
  ...
  echo "curl or wget is required to install AICodex." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/leagsoft/aicodex/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}
...
release_metadata_url() {
  ...
  printf 'https://api.github.com/repos/leagsoft/aicodex/releases/tags/rust-v%s\n' "$resolved_version"
}
...
resolve_metadata_version() {
  ...
  if [ -z "$metadata_version" ]; then
    echo "Failed to resolve the latest AICodex release version." >&2
    exit 1
  fi
...
resolve_release_from_github() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/leagsoft/aicodex/releases/latest"
  else
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for AICodex $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi
...
select_release_assets() {
  ...
  if release_asset_exists "$package_asset" &&
    ...
    echo "Could not find AICodex package or platform npm release assets for AICodex $resolved_version." >&2
    exit 1
  fi
...
file_sha256() {
  ...
  if command -v openssl >/dev/null 2>&1; then
    ...
    echo "sha256sum, shasum, or openssl is required to verify the AICodex download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded AICodex archive checksum did not match release metadata." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install AICodex." >&2
    exit 1
  fi
}
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.aicodex.*' -exec rm -f {} +
  fi
}
...
current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/aicodex" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/aicodex" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_aicodex() {
  command -v aicodex 2>/dev/null || true
}
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && aicodex"
      step "Future terminals: open a new terminal and run: aicodex"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: aicodex"
      step "Future terminals: open a new terminal and run: aicodex"
      ;;
  esac
}

maybe_launch_aicodex_now() {
  if prompt_yes_no "Start AICodex now?"; then
    step "Launching AICodex"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_aicodex)"
  ...
  step "Detected existing $manager-managed AICodex at $existing_path"
  warn "Multiple managed AICodex installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask aicodex"
      ;;
    bun)
      uninstall_cmd="bun remove -g @leagsoft/aicodex"
      ;;
    *)
      uninstall_cmd="npm uninstall -g @leagsoft/aicodex"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed AICodex now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed AICodex. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed AICodex installed. PATH order will determine which aicodex runs."
  fi
}
...
install_legacy_platform_npm_release() {
  ...
  cp "$vendor_root/aicodex/aicodex" "$stage_release/aicodex"
  cp "$vendor_root/path/rg" "$stage_release/codex-resources/rg"
  chmod 0755 "$stage_release/aicodex" "$stage_release/codex-resources/rg"
...
release_dir_is_complete() {
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/codex-package.json" ] &&
        [ -x "$release_dir/bin/aicodex" ] &&
        [ -x "$release_dir/bin/codex-code-mode-host" ] &&
        [ -x "$release_dir/aicodex" ] &&
  ...
  installed_version="$(version_from_binary "$release_dir/bin/aicodex" || version_from_binary "$release_dir/aicodex" || true)"
...
release_aicodex_relative_path() {
  release_dir="$1"

  if [ -x "$release_dir/bin/aicodex" ]; then
    printf 'bin/aicodex\n'
  else
    printf 'aicodex\n'
  fi
...
update_visible_command() {
  release_dir="$1"
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.aicodex.$$"
  aicodex_relative_path="$(release_aicodex_relative_path "$release_dir")"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/$aicodex_relative_path" "$tmp_link"
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating AICodex CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating AICodex CLI"
else
  step "Installing AICodex CLI"
fi
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  echo "Installed AICodex command did not report expected version $resolved_version." >&2
  exit 1
fi
...
printf 'AICodex CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_aicodex_now
